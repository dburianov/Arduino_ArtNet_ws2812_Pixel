...
byte ip[] = {10, 239, 238, 3}; // the IP adress of your device, that should be in same universe of the network you are using
...
const int number_of_channels=450; //512 for 512 channels
  ...
  if(packetSize>art_net_header_size && packetSize<=max_packet_size) {//check size to avoid unneeded checks
    ...
    if(match_artnet==1) { 
      ...
      if(is_opcode_is_dmx=1) {
        ...
        if(incoming_universe==select_universe2) setLEDS(150);
        if(incoming_universe==select_universe3) setLEDS(300);
  ...
  int j=startled;
            for(int i=start_address;i< number_of_channels;i=i+3) {
              if (j<=NUM_LEDS){
                ...
                leds[j].g = byte(packetBuffer[i+art_net_header_size+3]);
                leds[j].b = byte(packetBuffer[i+art_net_header_size+2]);
              }
              j++;
